...
class CorporateQuotePdfGenerator
{
    /**
     * @param  list<array<string, mixed>>  $detailsQuote
     */
    public static function generateForQuote(CorporateQuote $record, array $detailsQuote = []): bool
    {
    ...
    public static function regenerateIfMissing(CorporateQuote $record): bool
    {
     ...
     * @param  list<array<string, mixed>>  $detailsQuote
     */
    private static function generateMultiple(CorporateQuote $record, array $detailsQuote, string $agentName): bool
    {
        ...
        if ($planIds->isEmpty()) {
            $planIds = DB::table('detail_corporate_quotes')
    ...
    private static function buildDetailsPayload(
        CorporateQuote $record,
        int $planId,
        string $layout,
        string $agentName,
    ): ?array {
        $query = DB::table('detail_corporate_quotes')
            ...
            ->where('corporate_quote_id', $record->id)
            ->where('detail_corporate_quotes.plan_id', $planId);

        if (IndividualQuotePdfLayout::usesCoverageBreakdown($layout)) {
            $query->join('coverages', 'detail_corporate_quotes.coverage_id', '=', 'coverages.id')
    ...
    private static function resolveAgentName(CorporateQuote $record): string
    {
