...
final class AgentAddressClipboardFormat
{
    public static function canCopyVenezuela(Agent $record): bool
    {
        return filled($record->name)
            || filled($record->agency?->name_corporative)
    ...
    public static function canCopyInternational(Agent $record): bool
    {
        return filled($record->name)
            || filled($record->agency?->name_corporative)
    ...
    public static function venezuela(Agent $record): string
    {
    ...
    public static function international(Agent $record): string
    {
        $lines = self::headerLines($record);
     ...
     * @return list<string>
     */
    private static function headerLines(Agent $record): array
    {
        $lines = [];

        if (filled($record->name)) {
            $lines[] = trim((string) $record->name);
        }

        if (filled($record->agency?->name_corporative)) {
            $lines[] = 'Agencia '.trim((string) $record->agency->name_corporative);
        }
