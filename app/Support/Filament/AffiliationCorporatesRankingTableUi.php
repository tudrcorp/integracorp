...
class AffiliationCorporatesRankingTableUi
{
    /**
    ...
    public static function apply(
        ...
        return $table
            ...
            ->defaultSort('total_affiliations', 'desc')
            ->searchable(false)
            ->striped()
            ...
            ->extraAttributes([
                'class' => $tableClass,
                'data-ac-ranking-variant' => $variant,
            ])
            ->columns($columns)
    ...
    public static function heading(string $variant): string
    {
        return $variant === 'agency'
            ? 'Afiliaciones por agencia'
            : 'Afiliaciones por agente';
    }
    ...
    protected static function typeColumn(string $typeRelation, string $variant): TextColumn
    {
        return TextColumn::make($typeRelation.'.definition')
            ...
            ->sortable()
            ->placeholder('—')
            ->color(fn (?string $state): string => IndividualQuotesRankingTableUi::typeBadgeColor($state, $variant))
            ->extraAttributes(['class' => 'iq-ranking-type-badge']);
    }

    protected static function totalColumn(string $variant): TextColumn
    {
        return TextColumn::make('total_affiliations')
            ->label('Total')
            ->numeric()
            ->sortable()
            ->alignEnd()
