...
final class OperationInventoryEntryExportCsvController extends Controller
{
    private const CACHE_PREFIX = 'operation_inventory_entry_export_csv_';
    ...
    public function __invoke(Request $request): StreamedResponse
    {
        ...
        $filename = 'entradas_inventario_'.now()->format('Y-m-d_His').'.csv';
        ...
        return new StreamedResponse(function () use ($ids): void {
            ...
            OperationInventoryEntry::query()
                ->with(['product:id,code,name', 'ubication:id,name', 'operationInventory:id,name,barcode,ubication'])
                ->whereIn('id', $ids)
                ->orderBy('id')
                ->lazyById(200)
                ->each(function (OperationInventoryEntry $record) use ($handle): void {
                    fputcsv($handle, [
