...
final class AffiliationCorporateExportCsvController extends Controller
{
    private const CACHE_PREFIX = 'affiliation_corporate_export_csv_';
    ...
    public function __invoke(Request $request): StreamedResponse
    {
        ...
        return app(AffiliationCorporateCsvExportService::class)->streamCsv($filters, $panel);
    }
    ...
    /**
     * @return array{0: array{plan_id: ?int, status: ?string, affiliation_corporate_ids: ?list<int>}, 1: string, 2: string, 3: string}
     */
    private static function resolveExportContext(Request $request): array
    {
        ...
        $validated = $request->validate([
            ...
            'status' => ['nullable', 'string', Rule::in(array_keys(AffiliationCorporateCsvExportService::statusOptions()))],
        ]);
    ...
    private static function routeConfig(string $routeName): array
    {
        return match ($routeName) {
            'business.affiliation-corporates.export-csv' => [
                'business',
                'AUDIT_BUSINESS_AFFILIATION_CORPORATES_CSV_EXPORT',
                'business.affiliation-corporates.export-csv',
            ],
            'administration.affiliation-corporates.export-csv' => [
                'administration',
                'AUDIT_ADMINISTRATION_AFFILIATION_CORPORATES_CSV_EXPORT',
                'administration.affiliation-corporates.export-csv',
            ],
            'operations.affiliation-corporates.export-csv' => [
                'operations',
                'AUDIT_OPERATIONS_AFFILIATION_CORPORATES_CSV_EXPORT',
                'operations.affiliation-corporates.export-csv',
            ],
            default => abort(404),
        };
