        ...
        try {
            ...
            if (! isset($data['collections'])) {
                ...
                if ($record->affiliation->payment_frequency == 'ANUAL') {
                    ...
                    AnnualCollectionController::createAnnualCollectionAnual($collections);
                ...
                if ($record->affiliation->payment_frequency == 'TRIMESTRAL') {
                    ...
                    for ($i = 0; $i < $trimestral; $i++) {
                        ...
                        // Agregamos la fecha de pago a la array
                        $array_dates[] = Carbon::createFromFormat('d/m/Y', $collections->next_payment_date)->format('d/m/Y');
                    }

                    AnnualCollectionController::createAnnualCollectionTrimestral($collections, $array_dates);
                }
                ...
                if ($record->affiliation->payment_frequency == 'SEMESTRAL') {
                    ...
                    // Calculamos la fecha de pago semestral record->affiliation->activated_at + 6 meses
                    $collections->next_payment_date = Carbon::createFromFormat('d/m/Y', $record->affiliation->activated_at)->addMonths(6)->format('d/m/Y');
                    ...
                    AnnualCollectionController::createAnnualCollectionSemestral($collections);
                ...
                if ($record->affiliation->payment_frequency == 'MENSUAL') {
                    ...
                    AnnualCollectionController::createAnnualCollectionMensual($collections, $array_dates);
                }
            ...
            if (isset($data['collections']) && count($data['collections']) > 0) {
                ...
                /**Actualizo el status de los avisos de cobro en la tabla de collections*/
                for ($i = 0; $i < count($data['collections']); $i++) {

                    $collection = Collection::find($data['collections'][$i]);
                    $annualCollection = AnnualCollection::where('affiliation_code', $collection->affiliation_code)
                        ->where('sale_id', $collection->sale_id)
                        ->first();
                    if ($annualCollection) {
                        // tomo la fecha que el cliente esta pagando y busco el numero del mes de la fecha de pago
                        $annualCollection->update([
                            'month_'.AnnualCollectionController::extractMonth($collection->next_payment_date) => false,
                        ]);
                    }
                    // actualizo el status de la collection
                    $collection->sale_id = $sales->id;
                    $collection->status = 'PAGADO';
                    $collection->save();
                }

                WhiteCompanyCreditMovementRecorder::recordIndividualCollections($record, $data['collections']);
