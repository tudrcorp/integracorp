...
final class AffiliationExportCsvController extends Controller
{
    private const CACHE_PREFIX = 'affiliation_export_csv_';
    ...
    public function __invoke(Request $request): StreamedResponse
    {
        ...
        return app(AffiliationCsvExportService::class)->streamCsv($filters, $panel);
    }
    ...
    /**
     * @return array{0: array{plan_id: ?int, status: ?string, affiliation_ids: ?list<int>}, 1: string, 2: string, 3: string}
     */
    private static function resolveExportContext(Request $request): array
    {
        ...
        $validated = $request->validate([
            ...
            'status' => ['nullable', 'string', Rule::in(array_keys(AffiliationCsvExportService::statusOptions()))],
        ]);
    ...
    private static function routeConfig(string $routeName): array
    {
        return match ($routeName) {
            'business.affiliations.export-csv' => [
                'business',
                'AUDIT_BUSINESS_AFFILIATIONS_CSV_EXPORT',
                'business.affiliations.export-csv',
            ],
            'administration.affiliations.export-csv' => [
                'administration',
                'AUDIT_ADMINISTRATION_AFFILIATIONS_CSV_EXPORT',
                'administration.affiliations.export-csv',
            ],
            'operations.affiliations.export-csv' => [
                'operations',
                'AUDIT_OPERATIONS_AFFILIATIONS_CSV_EXPORT',
                'operations.affiliations.export-csv',
            ],
            default => abort(404),
        };
