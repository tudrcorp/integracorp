...
class ChatAgencyMasterRegistrationService
{
    /**
    ...
    public function register(array $payload): array
    {
        ...
        try {
            $registration = DB::transaction(function () use ($validated, $phone, $hashedPassword, $plainPassword): array {
                ...
                $agency->owner_code = AgencyController::resolveOwnerCodeForAgency(1, $identity['code']);
                $agency->code = $identity['code'];
                $agency->agency_type_id = 1;
                ...
                $user->link_agency = rtrim((string) config('app.url'), '/').'/m/o/c/'.Crypt::encryptString((string) $agency->code);
                $user->status = 'ACTIVO';
                ...
                return [
                    'registration_type' => 'agency_master',
            ...
            return [
                'success' => false,
                'message' => 'No pudimos completar el registro de la agencia master. Intenta nuevamente o contacta al equipo de negocios.',
            ];
        }

        return [
            'success' => true,
            'message' => 'Agencia master registrada exitosamente.',
            'data' => $registration,
        ];
    }
    ...
    public function portalLoginUrl(): string
    {
        return (string) config(
            'services.chat_agency_master_registration.portal_login_url',
            rtrim((string) config('app.url'), '/').'/master/login',
        );
    }
    ...
    public function queueRegistrationPackageViaWhatsApp(array $credentials): bool
    {
        ...
        if ($phone === '' || $agencyId <= 0 || $name === '') {
            Log::warning('PUBLIC-CHAT: No se encoló WhatsApp registro agencia master por datos incompletos.', [
                'agency_id' => $agencyId,
                'has_phone' => $phone !== '',
                'has_name' => $name !== '',
            ]);
        ...
        SendChatAgencyMasterRegistrationWhatsAppJob::dispatch([
            ...$credentials,
            'whatsapp_registration_queued' => true,
        ])->afterResponse();

        Log::info('PUBLIC-CHAT: WhatsApp registro agencia master encolado.', [
            'agency_id' => $agencyId,
            'phone' => $phone,
        ]);
    ...
    public function sendRegistrationPackageViaWhatsApp(array $credentials): bool
    {
        ...
        if ($phone === '' || $agencyId <= 0 || $name === '' || $agencyCode === '') {
            Log::warning('PUBLIC-CHAT: WhatsApp registro agencia master sin datos completos.', [
                'agency_id' => $agencyId,
                'has_phone' => $phone !== '',
                'has_name' => $name !== '',
            ]);
        ...
        Log::info('PUBLIC-CHAT: Envío WhatsApp registro agencia master.', [
            'agency_id' => $agencyId,
            'text_sent' => $textSent,
...
Tu registro de Agencia Master fue exitoso. Estos son tus datos de acceso:
...
TEXT,
    ...
    {
        return 'chat-agency-master-welcome/'.$this->welcomeLetterFilename($agencyCode);
    }
    ...
    public function ensureWelcomeLetterPdf(string $agencyCode, string $name): string
    {
        ...
        $legacyRelativePath = $this->welcomeLetterFilename($agencyCode);
