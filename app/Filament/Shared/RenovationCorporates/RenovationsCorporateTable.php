...
namespace App\Filament\Shared\RenovationCorporates;

use App\Http\Controllers\RenovationCorporateExportCsvController;
...
class RenovationsCorporateTable
{
    ...
    /**
     * @param  class-string<resource>  $renovationCorporateResourceClass
     * @param  class-string<resource>  $affiliationCorporateResourceClass
     */
    public static function configure(
        Table $table,
        string $renovationCorporateResourceClass,
        string $affiliationCorporateResourceClass,
        ?string $exportRouteName = null,
    ): Table {
        $table = $table
            ->heading('Renovaciones corporativas')
            ...
            ->emptyStateDescription('El proceso diario genera registros para afiliaciones corporativas ACTIVA con fecha de vigencia. Si acaba de ejecutarse, espere la próxima corrida o contacte a sistemas.')
            ...
            ->columns([
                ColumnGroup::make('Afiliación y vencimiento', [
                    TextColumn::make('code_affiliation')
                        ...
                        ->action(
                            ViewAction::make('viewFromCode')
                                ->url(fn (RenovationCorporate $record): string => $renovationCorporateResourceClass::getUrl('view', ['record' => $record])),
                        ),
                    TextColumn::make('affiliationCorporate.name_corporate')
                        ...
                        ->sortable()
                        ->limit(28)
                        ->tooltip(fn (RenovationCorporate $record): ?string => filled($record->affiliationCorporate?->name_corporate)
                            ? (string) $record->affiliationCorporate->name_corporate
                            : null)
                        ->placeholder('—')
                        ->toggleable(),
                    TextColumn::make('affiliationCorporate.rif')
                        ->label('RIF')
                    ...
                    TextColumn::make('remaining_days')
                        ...
                        ->alignCenter()
                        ->sortable()
                        ->icon(fn (RenovationCorporate $record): Heroicon => self::remainingDaysIcon($record))
                        ->iconColor(fn (RenovationCorporate $record): string => self::remainingDaysColor($record))
                        ->badge()
                        ->color(fn (RenovationCorporate $record): string => self::remainingDaysColor($record))
                        ->state(fn (RenovationCorporate $record): string => self::remainingDaysLabel($record))
                        ->description(fn (RenovationCorporate $record): string => self::remainingDaysDescription($record)),
                    ...
                    TextColumn::make('date_renewal')
                        ...
                        ->date('d/m/Y')
                        ->description(fn (RenovationCorporate $record): ?string => $record->date_renewal?->diffForHumans())
                        ->sortable(),
                ])
                    ->extraHeaderAttributes(['class' => self::COLUMN_GROUP_HEADER_CLASS]),
                ColumnGroup::make('Expediente vigente (antes)', [
                    TextColumn::make('affiliationCorporate.effective_date')
                        ->label('Vigencia desde')
                        ->icon(Heroicon::OutlinedCalendar)
                        ->state(fn (RenovationCorporate $record): string => FilamentDateDisplay::toDmy($record->affiliationCorporate?->effective_date) ?? '—')
                        ->description('Expediente actual')
                        ->placeholder('—'),
                    TextColumn::make('current_plan_summary')
                        ->label('Plan actual')
                        ->icon(Heroicon::OutlinedDocumentText)
                        ->badge()
                        ->state(fn (RenovationCorporate $record): string => self::currentPlanLabel($record))
                        ->color(fn (RenovationCorporate $record): string => self::planBadgeColorForPlanId(
                            (int) ($record->previous_plan_id ?? $record->plan_id ?? 0),
                        ))
                        ->placeholder('—'),
                    TextColumn::make('current_coverage_summary')
                        ->label('Cobertura actual')
                        ->icon(Heroicon::OutlinedShieldCheck)
                        ->state(fn (RenovationCorporate $record): string => self::coverageLabel(
                            $record->coverage_id,
                            $record->coverage?->price,
                        ))
                        ->alignEnd(),
                    TextColumn::make('affiliationCorporate.fee_anual')
                        ->label('Anual vigente')
                        ->icon(Heroicon::OutlinedBanknotes)
                        ->money('USD')
                        ->alignEnd()
                        ->weight('medium')
                        ->description(fn (RenovationCorporate $record): ?string => filled($record->affiliationCorporate?->total_amount)
                            ? 'Pago: US$ '.number_format((float) $record->affiliationCorporate->total_amount, 2)
                            : null),
                ])
                ...
                ColumnGroup::make('Si acepta renovación', [
                    TextColumn::make('projected_effective_date')
                        ...
                        ->icon(Heroicon::OutlinedCalendarDays)
                        ->state(fn (RenovationCorporate $record): string => $record->date_renewal?->format('d/m/Y') ?? '—')
                    ...
                    TextColumn::make('plan.description')
                        ...
                        ->badge()
                        ->color(fn (RenovationCorporate $record): string => self::planBadgeColor($record))
                        ->description(fn (RenovationCorporate $record): ?string => self::projectedPlanDescription($record))
                        ->placeholder('—')
                    ...
                    TextColumn::make('subtotal_anual')
                        ...
                        ->weight('bold')
                        ->alignEnd()
                        ->color(fn (RenovationCorporate $record): string => self::annualDeltaColor($record))
                        ->description(fn (RenovationCorporate $record): string => 'Referencia: US$ '.number_format((float) $record->fee, 2))
                        ->sortable(),
                    TextColumn::make('age')
                        ...
                        ->alignCenter()
                        ->suffix(' años')
                        ->description(fn (RenovationCorporate $record): ?string => $record->birth_date
                    ...
                    TextColumn::make('negotiation_flag')
                        ->label('Negociación')
                        ->alignCenter()
                        ->state(fn (RenovationCorporate $record): string => $record->is_negotiation_candidate
                            ? 'Plan Especial'
                            : '—')
                        ->icon(fn (RenovationCorporate $record): ?Heroicon => $record->is_negotiation_candidate
                            ? Heroicon::OutlinedChatBubbleLeftRight
                            : null)
                        ->badge()
                        ->color(fn (RenovationCorporate $record): string => $record->is_negotiation_candidate ? 'warning' : 'gray')
                        ->placeholder('—')
                        ->extraAttributes(fn (RenovationCorporate $record): array => $record->is_negotiation_candidate && filled($record->negotiation_notes)
                            ? [
                ...
                ColumnGroup::make('Variación', [
                    TextColumn::make('renewal_delta_summary')
                        ...
                        ->state(fn (RenovationCorporate $record): string => self::annualDeltaLabel($record))
                        ->description(fn (RenovationCorporate $record): string => self::renewalChangeDescription($record))
                        ->badge()
                        ->color(fn (RenovationCorporate $record): string => self::annualDeltaColor($record))
                        ->alignCenter(),
                ...
                ColumnGroup::make('Estructura comercial', [
                    TextColumn::make('affiliationCorporate.agency.name_corporative')
                        ...
                        ->sortable()
                        ->limit(32)
                        ->description(fn (RenovationCorporate $record): ?string => filled($record->code_agency)
                            ? (string) $record->code_agency
                            : null)
                        ->tooltip(fn (RenovationCorporate $record): ?string => filled($record->affiliationCorporate?->agency?->name_corporative)
                            ? (string) $record->affiliationCorporate->agency->name_corporative
                            : null)
                        ->placeholder('—'),
                    TextColumn::make('affiliationCorporate.agent.name')
                        ...
                        ->sortable()
                        ->limit(28)
                        ->description(fn (RenovationCorporate $record): ?string => filled($record->agent_id)
                            ? (string) $record->agent_id
                            : null)
                        ->tooltip(fn (RenovationCorporate $record): ?string => filled($record->affiliationCorporate?->agent?->name)
                            ? (string) $record->affiliationCorporate->agent->name
                            : null)
                        ->placeholder('—'),
                ...
                ColumnGroup::make('Auditoría', [
                    TextColumn::make('updated_at')
                        ...
                        ->dateTime('d/m/Y H:i')
                        ->description(fn (RenovationCorporate $record): string => $record->updated_at->diffForHumans())
                        ->sortable()
                        ->toggleable(isToggledHiddenByDefault: true),
                ])
                    ->extraHeaderAttributes(['class' => self::COLUMN_GROUP_HEADER_CLASS]),
            ])
            ->recordClasses(fn (RenovationCorporate $record): array => self::recordRowClasses($record))
            ->filters([
            ...
            ->recordActions([
                ActionGroup::make([
                    ...
                    Action::make('viewAffiliation')
                        ...
                        ->color('info')
                        ->url(fn (RenovationCorporate $record): string => $affiliationCorporateResourceClass::getUrl('view', ['record' => $record->affiliation_corporate_id]))
                ...
                BulkActionGroup::make([
                    BulkAction::make('exportCsvController')
                        ...
                        ->action(function (Collection $records) use ($exportRouteName) {
                            ...
                            $token = RenovationCorporateExportCsvController::storeIdsAndGetToken($ids);
    ...
    private static function acceptRenovationAction(): Action
    {
        return self::configureAcceptModal(
            Action::make('acceptRenovation')
                ...
                ->visible(fn (RenovationCorporate $record): bool => $record->status === 'PERIODO DE RENOVACION')
                ->fillForm(fn (RenovationCorporate $record): array => [
                    'plan_id' => $record->plan_id,
                    'age_range_id' => $record->age_range_id,
                    'coverage_id' => $record->coverage_id,
                    'payment_frequency' => $record->payment_frequency ?? 'ANUAL',
                ])
                ->form(fn (RenovationCorporate $record): array => AcceptRenovationActionForm::schema(collect([$record])))
                ->action(function (RenovationCorporate $record, array $data): void {
        ...
        return self::configureAcceptModal(
            BulkAction::make('acceptRenovations')
                ...
                ->fillForm(function (Collection $records): array {
                    /** @var RenovationCorporate|null $reference */
    ...
    /**
     * @param  Collection<int, RenovationCorporate>  $records
    ...
    private static function processAcceptRenovations(Collection $records, array $data = []): void
    {
        ...
        $result = app(AcceptAffiliationCorporateRenovationsService::class)->accept($records, $acceptedBy, $manualOptions);
    ...
    private static function remainingDaysColor(RenovationCorporate $record): string
    {
    ...
    private static function remainingDaysIcon(RenovationCorporate $record): Heroicon
    {
        if ($record->remaining_days < 0) {
    ...
    private static function remainingDaysLabel(RenovationCorporate $record): string
    {
    ...
    private static function remainingDaysDescription(RenovationCorporate $record): string
    {
        if ($record->remaining_days === null) {
            return 'Sin conteo';
        }
    ...
    private static function planBadgeColor(RenovationCorporate $record): string
    {
    ...
    private static function currentPlanLabel(RenovationCorporate $record): string
    {
        if ($record->is_negotiation_candidate && filled($record->previous_plan_id)) {
            return (string) ($record->previousPlan?->description ?? 'Plan Ideal');
        }

        return (string) ($record->plan?->description ?? '—');
    }

    private static function projectedPlanDescription(RenovationCorporate $record): ?string
    {
        if ($record->is_negotiation_candidate) {
            return 'Cambio a Plan Especial';
        }

        return 'Propuesta de renovación';
    }

    private static function annualDeltaLabel(RenovationCorporate $record): string
    {
        $current = (float) ($record->affiliationCorporate?->fee_anual ?? 0);
    ...
    private static function annualDeltaColor(RenovationCorporate $record): string
    {
        $current = (float) ($record->affiliationCorporate?->fee_anual ?? 0);
        $projected = (float) $record->subtotal_anual;
        $delta = round($projected - $current, 2);
    ...
    private static function renewalChangeDescription(RenovationCorporate $record): string
    {
        $parts = [];

        $currentPlan = (string) ($record->affiliationCorporate?->plan?->description ?? '—');
        ...
        $currentVigencia = FilamentDateDisplay::toDmy($record->affiliationCorporate?->effective_date) ?? '—';
     ...
     * @return list<string>
     */
    private static function recordRowClasses(RenovationCorporate $record): array
    {
