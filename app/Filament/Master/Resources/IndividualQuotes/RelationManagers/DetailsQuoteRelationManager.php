                    ...
                    BulkAction::make('quote_multiple')
                        ...
                        ->action(function (Collection $records, RelationManager $livewire) {
                            try {
                                ...
                                return redirect()->route('filament.master.resources.affiliations.create', [
                                    'id' => $livewire->ownerRecord->id,
                                    'plan_id' => $record->plan_id,
                                    'coverage_id' => $record->coverage_id,
                                ]);
                            } catch (\Throwable $th) {
                                Log::error('MASTER: Falla al generar preafiliación desde detalle de cotización individual', [
