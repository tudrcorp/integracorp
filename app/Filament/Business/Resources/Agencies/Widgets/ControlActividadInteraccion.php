...
class ControlActividadInteraccion extends Widget
{
    ...
    protected string $view = 'filament.widgets.control-actividad-interaccion-agencias';
    ...
    public function getTrafficLightStats(): array
    {
        ...
        $agencies = AgencyActivityQuery::applyToAgenciesTableQuery(DB::table('agencies'), $asOf)
            ->addSelect(['agencies.id'])
            ->get();

        $total = (int) $agencies->count();
        ...
        foreach ($agencies as $agency) {
            $lastInteractionAt = $agency->last_interaction_at ? Carbon::parse($agency->last_interaction_at) : null;
            $daysSinceInteraction = $lastInteractionAt ? $lastInteractionAt->diffInDays($asOf) : 9999;

            $lastSaleAt = $agency->last_sale_at ? Carbon::parse($agency->last_sale_at) : null;
