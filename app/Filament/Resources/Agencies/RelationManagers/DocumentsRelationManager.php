...
class DocumentsRelationManager extends RelationManager
{
            ...
            Section::make('CARGA DE DOCUMENTOS')
                ...
                ->schema([
                    ...
                    FileUpload::make('fir_dig_agency')
                        ...
                        ->uploadingMessage('Cargando firma...')
                        ->hidden(fn() => AgencyDocument::where('agency_id', $this->getOwnerRecord()->id)->where('title', 'FIRMA DIGITAL AGENCIA')->where('document', '!=', null)->exists()),
                    FileUpload::make('file_ci_rif')
                        ->label('CI/RIF')
                        ->uploadingMessage('Cargando documento...')
                        ->hidden(fn() => AgencyDocument::where('agency_id', $this->getOwnerRecord()->id)->where('title', 'DOCUMENTO DE IDENTIDAD CI/RIF')->where('document', '!=', null)->exists()),
                    ...
                    FileUpload::make('file_account_zelle')
                        ->label('Cta. Zelle')
                        ->uploadingMessage('Cargando documento...')
                        ->hidden(fn() => AgencyDocument::where('agency_id', $this->getOwnerRecord()->id)->where('title', 'CUENTA ZELLE')->where('document', '!=', null)->exists()),
    ...
    {
        return $table
        ->heading('Documentos consignados de la agencia')
            ->columns([
                Stack::make([
            ...
            ->recordActions([
                ...
                DeleteAction::make()
                    ...
                    ->action(function (AgencyDocument $record) {
                        $record->delete();
                    })
                ]);
    }
