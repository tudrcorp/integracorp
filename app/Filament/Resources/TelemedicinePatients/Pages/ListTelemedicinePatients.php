...
class ListTelemedicinePatients extends ListRecords
{
                                            ...
                                            Select::make('affiliate_corporate_id')
                                                ...
                                                ->options(
                                                    AffiliateCorporate::query()
                                                        ...
                                                        ->orderBy('first_name')
                                                        ->get()
                                                        ->mapWithKeys(fn (AffiliateCorporate $affiliate): array => [
                                                            $affiliate->id => self::corporateAffiliateOptionLabel($affiliate),
                                                        ])
                                                        ->all()
                                                )
                                                ->searchable()
                ...
                ->action(function (array $data) {
                    ...
                    if ($data['type_affiliate'] == 'cor') {
                        ...
                        $patient = TelemedicinePatient::create([
                            ...
                            // Informacion del Afiliado -> Paciente (identidad propia; no reutilizar otro paciente)
                            'full_name' => self::corporateAffiliateFullName($affiliation[0]),
                            'nro_identificacion' => $affiliation[0]['nro_identificacion'],
                            'birth_date' => $affiliation[0]['birth_date'],
                            'sex' => $affiliation[0]['sex'],
                            'age' => $affiliation[0]['age'],
    ...
    private static function corporateAffiliateOptionLabel(AffiliateCorporate $affiliate): string
    {
        $fullName = trim(implode(' ', array_filter([
            trim((string) $affiliate->first_name),
            trim((string) $affiliate->last_name),
        ])));

        $document = trim((string) $affiliate->nro_identificacion);

        if ($document === '') {
            return $fullName !== '' ? $fullName : (string) $affiliate->id;
        }

        return ($fullName !== '' ? $fullName : 'Sin nombre').' — '.$document;
    }

    /**
     * @param  array<string, mixed>  $affiliate
     */
    private static function corporateAffiliateFullName(array $affiliate): string
    {
