            ...
            ->headerActions([
                ...
                Action::make('create_corporate_quote')
                    ...
                    ->action(function (RelationManager $livewire, array $data) {
                        ...
                        //             $detail_individual_quote->corporate_quote_id    = $corporate_quote->id;
                        //             $detail_individual_quote->plan_id               = $resultado[$i]['plan_id'];
                        //             $detail_individual_quote->age_range_id          = $resultado[$i]['age_range_id'];
                        //             $detail_individual_quote->coverage_id           = $fee->coverage_id;
                        //             $detail_individual_quote->fee                   = $fee->price;
                        //             $detail_individual_quote->total_persons         = $resultado[$i]['total_persons'];
                        //             $detail_individual_quote->subtotal_anual        = $resultado[$i]['total_persons'] * $fee->price;
                        //             $detail_individual_quote->subtotal_quarterly    = ($resultado[$i]['total_persons'] * $fee->price) / 4;
                        //             $detail_individual_quote->subtotal_biannual     = ($resultado[$i]['total_persons'] * $fee->price) / 2;
                        //             $detail_individual_quote->subtotal_monthly      = ($resultado[$i]['total_persons'] * $fee->price) / 12;
                        ...
                        //     Notification::make()
                        //         ->success()
                        //         ->title('Cotización corporativa creada con éxito')
                        //         ->send();
