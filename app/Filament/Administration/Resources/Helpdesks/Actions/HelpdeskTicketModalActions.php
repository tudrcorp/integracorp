    ...
    public static function makeAddNoteAction(): Action
    {
        return Action::make('addNote')
            ...
            ->action(function (HelpDesk $record, array $data): void {
                ...
                if ($plainLength < 3) {
                    SecurityAudit::log('AUDIT_HELPDESK_NOTE_ADD_FAILED', 'administration.helpdesks.add-note', [
        ...
        return Action::make('updateStatus')
            ...
            ->action(function (HelpDesk $record, array $data): void {
                ...
                if ($sanitized === $record->status) {
                    SecurityAudit::log('AUDIT_HELPDESK_STATUS_UPDATE_SKIPPED', 'administration.helpdesks.update-status', [
                        'panel' => 'administration',
                        'helpdesk_id' => $record->getKey(),
                        'status' => $record->status,
                ...
                if ($isAssignee) {
                    ...
                    if ($explanationError !== null) {
                        SecurityAudit::log('AUDIT_HELPDESK_STATUS_UPDATE_FAILED', 'administration.helpdesks.update-status', [
                            'panel' => 'administration',
                            'helpdesk_id' => $record->getKey(),
        ...
        return Action::make('updatePriority')
            ...
            ->action(function (HelpDesk $record, array $data): void {
                ...
                if (! HelpdeskResource::currentUserIsHelpdeskTicketCreator($record)) {
                    SecurityAudit::log('AUDIT_HELPDESK_PRIORITY_UPDATE_DENIED', 'administration.helpdesks.update-priority', [
                        'panel' => 'administration',
                        'helpdesk_id' => $record->getKey(),
                ...
                if (! in_array($newPriority, $allowed, true)) {
                    SecurityAudit::log('AUDIT_HELPDESK_PRIORITY_UPDATE_FAILED', 'administration.helpdesks.update-priority', [
                        'panel' => 'administration',
                        'helpdesk_id' => $record->getKey(),
                ...
                if ($newPriority === $previousPriority) {
                    SecurityAudit::log('AUDIT_HELPDESK_PRIORITY_UPDATE_SKIPPED', 'administration.helpdesks.update-priority', [
                        'panel' => 'administration',
                        'helpdesk_id' => $record->getKey(),
                        'priority' => $record->priority,
                        'updated_by' => $user->name,
                ...
                $whatsAppReport = HelpdeskTicketAssigneeWhatsAppService::dispatchCustomMessageToEachAssigneeWithReport(
                    ...
                    auditRoute: 'administration.helpdesks.notifications.whatsapp.priority',
                );
                ...
                SecurityAudit::log('AUDIT_HELPDESK_PRIORITY_UPDATED', 'administration.helpdesks.update-priority', [
                    'panel' => 'administration',
                    'helpdesk_id' => $record->getKey(),
                    'old_priority' => $previousPriority,
                    'new_priority' => $newPriority,
