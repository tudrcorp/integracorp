...
class CorretajeAgentsRegistrationMomStats extends Widget
{
    ...
    public function getComparisonViewData(): array
    {
        ...
        return [
            ...
            'registered_period_label' => 'Registrados este mes',
            'wire_key_prefix' => 'agents-mom',
            'cards' => [
                $this->mapCard(
                    key: 'superiors',
                    title: 'Agentes SUPERIORES',
                    accent: 'sky',
                    comparison: $payload['superiors'],
                    year: (int) $yearSeries['year'],
                    labels: $yearSeries['labels'],
                    values: $yearSeries['superiors'],
                ),
                $this->mapCard(
                    key: 'subagents',
                    title: 'SUBAGENTES',
                    accent: 'violet',
                    comparison: $payload['subagents'],
                    year: (int) $yearSeries['year'],
                    labels: $yearSeries['labels'],
                    values: $yearSeries['subagents'],
                ),
            ],
        ];
    }
    ...
    {
        try {
            return app(CorretajeAgentsMetricsClient::class)->registrationComparison();
        } catch (Throwable $exception) {
            Log::warning('No se pudo cargar la comparación MoM de agentes desde integracorp-api.', [
            ...
            return [
                ...
                'year_series' => [
                    ...
                    'superiors' => $emptySeries['values'],
                    'subagents' => $emptySeries['values'],
                ],
            ];
        }
    }
