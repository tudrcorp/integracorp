...
class SendIndividualQuoteDayTwelveFollowUp implements ShouldQueue
{
    use Dispatchable, InteractsWithQueue, Queueable, ReportsScheduledExecution, SerializesModels;

    private const FOLLOW_UP_LABEL = 'Seguimiento cotizaciones (12 días)';
    ...
    public function handle(): void
    {
        $this->runWithScheduledReport(
            'Seguimiento cotizaciones individuales (12 días)',
            function (): void {
                $this->dispatchFollowUpMessages();
            },
            'Envía recordatorio por WhatsApp y correo de cotizaciones individuales PRE-APROBADA creadas hace 12 días, informando vencimiento próximo.',
            [
    ...
    private function dispatchAllyWhatsAppMessages(Collection $quotes, string $body, string $ally): int
    {
        ...
        foreach ($rawPhones as $rawPhone) {
            ...
            try {
                ...
                Log::error('SendIndividualQuoteDayTwelveFollowUp: error despachando WhatsApp', [
                    'phone' => $phone,
    ...
    private function dispatchAllyEmailMessages(Collection $quotes, string $body, string $ally): int
    {
        ...
        foreach ($emails as $email) {
            try {
                ...
                Log::error('SendIndividualQuoteDayTwelveFollowUp: error enviando correo de aliado', [
                    'email' => $email,
                    'message' => $exception->getMessage(),
                ]);
            }
    ...
    private function dispatchClientFollowUpMessages(Collection $quotes, string $ally): array
    {
        ...
        foreach ($quotes as $quote) {
            $clientBody = IndividualQuoteDayTwelveFollowUp::clientWhatsappBody($quote);
    ...
    private function dispatchClientWhatsApp(
        ...
        try {
            SendNotificacionWhatsApp::dispatch(null, $clientBody, $phone, null, [
                'panel' => 'system',
                'source' => 'individual-quotes.day-twelve-follow-up.client',
                'ally' => $ally,
            ...
            Log::error('SendIndividualQuoteDayTwelveFollowUp: error despachando WhatsApp de cliente', [
                'phone' => $phone,
                'quote_code' => $quote->code,
                'message' => $exception->getMessage(),
            ]);
    ...
    private function dispatchClientEmail(
        ...
        try {
            ...
            Log::error('SendIndividualQuoteDayTwelveFollowUp: error enviando correo de cliente', [
                'email' => $email,
                'quote_code' => $quote->code,
                'message' => $exception->getMessage(),
            ]);
    ...
    {
        Log::error('SendIndividualQuoteDayTwelveFollowUp: FAILED', [
            'message' => $exception?->getMessage(),
