@php
    ...
    $shell = match ($tone) {
        'ok' => 'border-sky-200/80 bg-sky-50/40 dark:border-sky-500/25 dark:bg-sky-950/20',
        'warning' => 'border-amber-200/80 bg-amber-50/50 dark:border-amber-500/25 dark:bg-amber-950/20',
        'danger' => 'border-rose-200/80 bg-rose-50/45 dark:border-rose-500/25 dark:bg-rose-950/20',
...
<div class="rounded-2xl border {{ $shell }} px-3.5 py-3 sm:px-4">
    <div class="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1">
        <p class="text-[13px] font-semibold tracking-tight text-slate-800 dark:text-slate-100">Uso clínico y límites</p>
        @if (filled($summary))
    ...
    <p class="mt-0.5 text-[11px] leading-snug text-slate-500 dark:text-slate-400">
        Límite comercial · canal · usos. El médico descuenta al guardar la consulta.
    </p>
    ...
    @if ($rows !== [])
        <ul class="mt-2 divide-y divide-slate-200/80 dark:divide-white/10">
            @foreach ($rows as $row)
                <li class="flex items-start justify-between gap-3 py-1.5 first:pt-0 last:pb-0">
                    <div class="min-w-0">
                        ...
                        <p class="text-[11px] leading-snug text-slate-500 dark:text-slate-400">
                            ...
                            @if (filled($row['channel'] ?? null))
                                · {{ $row['channel'] }}
                            @endif
                            @if (filled($row['count'] ?? null))
                                · {{ $row['count'] }}
                            @endif
                        </p>
                    </div>
                    <span class="mt-0.5 shrink-0 rounded-full px-2 py-0.5 text-[11px] font-semibold {{ $pill($row['tone'] ?? 'muted') }}">
                        {{ $row['balance'] }}
                    </span>
                </li>
            @endforeach
    ...
    @if ($otpHint)
        <p class="mt-2 text-[11px] text-slate-500 dark:text-slate-400">Si el cupo está agotado, el médico de TDG pide OTP. Operaciones no excepciona.</p>
    @endif
</div>
