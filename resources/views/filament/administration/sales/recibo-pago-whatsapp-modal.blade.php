...
@php
    ...
    $agent = $recipients['agent'] ?? ['linked' => false, 'name' => null, 'code' => null, 'phone_display' => null];
    $agency = $recipients['agency'] ?? ['linked' => false, 'name' => null, 'code' => null, 'phone_display' => null];
    $internal = $recipients['internal'] ?? ['name' => 'Sol Rodriguez', 'phone_display' => '0414-3027250'];
    $targets = $recipients['targets'] ?? [];
    ...
    <div class="fi-scoped space-y-4">
        <article class="overflow-hidden rounded-3xl border border-gray-200/80 bg-white/80 shadow-sm backdrop-blur-md dark:border-white/10 dark:bg-gray-900/70">
            ...
            <div class="space-y-4 p-4">
                <section class="rounded-2xl border border-slate-200/80 bg-slate-50/80 p-4 dark:border-white/10 dark:bg-white/5">
                    <div class="mb-3 flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500 dark:text-gray-400">Teléfono registrado en la tabla de agentes</p>
                        </div>
                    </div>
                    ...
                    @else
                        <dl class="grid gap-2 text-sm">
                            ...
                            <div class="flex flex-wrap justify-between gap-2">
                                ...
                                <dd class="font-semibold {{ filled($agent['phone_display']) ? 'text-emerald-700 dark:text-emerald-300' : 'text-amber-700 dark:text-amber-300' }}">
                                    {{ $agent['phone_display'] ?: 'Sin teléfono registrado' }}
                                </dd>
                            </div>
                        </dl>
                    @endif
                ...
                <section class="rounded-2xl border border-slate-200/80 bg-slate-50/80 p-4 dark:border-white/10 dark:bg-white/5">
                    <div class="mb-3 flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500 dark:text-gray-400">Teléfono registrado en la tabla de agencias</p>
                        </div>
                    </div>
                    ...
                    @else
                        <dl class="grid gap-2 text-sm">
                            ...
                            <div class="flex flex-wrap justify-between gap-2">
                                <dt class="text-gray-500 dark:text-gray-400">Teléfono</dt>
                                <dd class="font-semibold {{ filled($agency['phone_display']) ? 'text-emerald-700 dark:text-emerald-300' : 'text-amber-700 dark:text-amber-300' }}">
                                    {{ $agency['phone_display'] ?: 'Sin teléfono registrado' }}
                                </dd>
                            </div>
                        </dl>
                    @endif
                ...
                <section class="rounded-2xl border border-sky-200/80 bg-sky-50/70 p-4 dark:border-sky-500/20 dark:bg-sky-950/20">
                    <p class="mb-2 text-sm font-semibold text-sky-900 dark:text-sky-200">
                        Copia interna
                    </p>
                    <dl class="grid gap-2 text-sm">
                        <div class="flex flex-wrap justify-between gap-2">
                            <dt class="text-gray-500 dark:text-gray-400">Nombre</dt>
                            <dd class="font-medium text-gray-900 dark:text-white">{{ $internal['name'] }}</dd>
                        </div>
                        <div class="flex flex-wrap justify-between gap-2">
                            <dt class="text-gray-500 dark:text-gray-400">Teléfono</dt>
                            <dd class="font-semibold text-emerald-700 dark:text-emerald-300">{{ $internal['phone_display'] }}</dd>
                        </div>
                    </dl>
                </section>
                ...
                <section class="rounded-2xl border border-emerald-200/80 bg-emerald-50/70 p-4 dark:border-emerald-500/20 dark:bg-emerald-950/20">
                    <p class="mb-2 text-sm font-semibold text-emerald-900 dark:text-emerald-200">
                        WhatsApp — números que recibirán el PDF
                    </p>
                        ...
                        <ul class="space-y-2">
                            @foreach ($targets as $target)
                                <li class="flex items-center gap-2 rounded-xl border border-emerald-200/70 bg-white/80 px-3 py-2 text-sm font-medium text-gray-800 dark:border-emerald-500/20 dark:bg-gray-900/60 dark:text-gray-100">
                                    <x-heroicon-o-chat-bubble-left-right class="size-4 shrink-0 text-emerald-600 dark:text-emerald-300" />
                                    <span>
                                        {{ $target['phone_display'] }}
                                        @if (filled($target['name']))
                                            <span class="text-gray-500 dark:text-gray-400">· {{ $target['name'] }}</span>
                                        @endif
                                    </span>
                                </li>
                            @endforeach
                    ...
                    @else
                        <p class="text-sm text-amber-700 dark:text-amber-300">
                            No hay números válidos disponibles para el envío.
                        </p>
                    @endif
                </section>
            </div>
