...
<div class="space-y-4 px-0.5 py-1">
    @if ($agent)
        <div class="overflow-hidden rounded-[1.25rem] border border-gray-200/80 bg-gray-50/90 shadow-inner ring-1 ring-black/[0.03] dark:border-white/10 dark:bg-white/[0.06] dark:ring-white/[0.05]">
            <div class="flex items-center justify-between gap-3 border-b border-gray-200/70 px-4 py-3.5 dark:border-white/10">
                <div class="flex min-w-0 items-center gap-3">
                    ...
                    <div class="min-w-0">
                        <p class="text-[0.65rem] font-semibold uppercase tracking-[0.12em] text-gray-500 dark:text-gray-400">Agente comercial</p>
                        <p class="truncate text-sm font-semibold tracking-tight text-gray-900 dark:text-white">{{ $agent->name ?: 'Sin nombre' }}</p>
                        <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">Codigo: {{ $agent->code_agent ?: 'N/A' }}</p>
                    </div>
                </div>
                <span class="shrink-0 rounded-full px-3 py-1.5 text-xs font-bold uppercase tracking-wide ring-1 {{ ($agent->status ?? null) === 'ACTIVO' ? 'bg-emerald-500/15 text-emerald-700 ring-emerald-500/25 dark:bg-emerald-400/15 dark:text-emerald-200 dark:ring-emerald-400/30' : 'bg-amber-500/15 text-amber-700 ring-amber-500/25 dark:bg-amber-400/15 dark:text-amber-200 dark:ring-amber-400/30' }}">
            ...
            <section class="rounded-[1.1rem] border border-gray-200/80 bg-white/90 p-4 shadow-sm dark:border-white/10 dark:bg-white/[0.04]">
                ...
                <dl class="space-y-2 text-sm">
                    <div class="flex items-start justify-between gap-2">
                        <dt class="font-medium text-gray-600 dark:text-gray-300">CI</dt>
                        <dd class="text-right text-gray-900 dark:text-white">{{ $agent->ci ?: 'N/A' }}</dd>
                    </div>
                    <div class="flex items-start justify-between gap-2">
                        <dt class="font-medium text-gray-600 dark:text-gray-300">RIF</dt>
                        <dd class="text-right text-gray-900 dark:text-white">{{ $agent->rif ?: 'N/A' }}</dd>
                    </div>
                    <div class="flex items-start justify-between gap-2">
                        <dt class="font-medium text-gray-600 dark:text-gray-300">Tipo de agente</dt>
                        <dd class="text-right text-gray-900 dark:text-white">{{ data_get($agent, 'typeAgent.description', 'N/A') }}</dd>
                    </div>
                </dl>
            </section>
            ...
            <section class="rounded-[1.1rem] border border-gray-200/80 bg-white/90 p-4 shadow-sm dark:border-white/10 dark:bg-white/[0.04]">
                ...
                <dl class="space-y-2 text-sm">
                    <div class="flex items-start justify-between gap-2">
                        ...
                        <dd class="text-right text-gray-900 dark:text-white">{{ $agent->phone ?: 'N/A' }}</dd>
                    </div>
                    <div class="flex items-start justify-between gap-2">
                        ...
                        <dd class="text-right text-gray-900 dark:text-white break-all">
                            @if (filled($agent->email))
                                <button
                                    ...
                                    x-data="{ copied: false }"
                                    x-on:click="
                                        navigator.clipboard?.writeText(@js($agent->email));
                                        copied = true;
                    ...
                    <div class="flex items-start justify-between gap-2">
                        ...
                        <dd class="text-right text-gray-900 dark:text-white">{{ $agent->user_instagram ?: 'N/A' }}</dd>
                    </div>
                </dl>
            </section>
        </div>
        ...
        <section class="rounded-[1.1rem] border border-gray-200/80 bg-white/90 p-4 shadow-sm dark:border-white/10 dark:bg-white/[0.04]">
            <p class="mb-2 text-[0.65rem] font-semibold uppercase tracking-[0.12em] text-gray-500 dark:text-gray-400">Contexto comercial</p>
            <dl class="grid gap-2 text-sm sm:grid-cols-2">
                <div class="flex items-start justify-between gap-2 sm:block">
                    <dt class="font-medium text-gray-600 dark:text-gray-300">Agencia asociada</dt>
                    <dd class="text-gray-900 dark:text-white">{{ data_get($agent, 'agency.name_corporative', 'N/A') }}</dd>
                </div>
                <div class="flex items-start justify-between gap-2 sm:block">
                    <dt class="font-medium text-gray-600 dark:text-gray-300">Pais</dt>
                    <dd class="text-gray-900 dark:text-white">{{ data_get($agent, 'country.name', 'N/A') }}</dd>
                </div>
                <div class="flex items-start justify-between gap-2 sm:block">
                    <dt class="font-medium text-gray-600 dark:text-gray-300">Estado</dt>
                    <dd class="text-gray-900 dark:text-white">{{ data_get($agent, 'state.name', 'N/A') }}</dd>
                </div>
                <div class="flex items-start justify-between gap-2 sm:block">
                    <dt class="font-medium text-gray-600 dark:text-gray-300">Ciudad</dt>
                    <dd class="text-gray-900 dark:text-white">{{ data_get($agent, 'city.name', 'N/A') }}</dd>
                </div>
            </dl>
        </section>
    @else
        <div class="rounded-[1.25rem] border border-dashed border-gray-300/90 bg-white/60 px-4 py-10 text-center text-sm text-gray-500 dark:border-white/15 dark:bg-white/[0.04] dark:text-gray-400">
            <p class="font-medium text-gray-600 dark:text-gray-300">Sin datos del agente</p>
            <p class="mt-1 text-xs">Esta venta no tiene un agente relacionado para mostrar.</p>
        </div>
    @endif
