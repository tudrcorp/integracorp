...
<div class="space-y-4">
    <div
        ...
        <div class="flex items-center justify-between gap-3 border-b border-gray-200/70 px-4 py-3.5 dark:border-white/10">
            <div class="flex min-w-0 items-center gap-3">
                <span
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-5 w-5 text-emerald-600 dark:text-emerald-400" aria-hidden="true">
        ...
        <div class="px-4 py-3">
            <p class="text-sm text-gray-600 dark:text-gray-300">
                Carga tutoriales en video u otros materiales de apoyo. Al guardar, quedan en esta biblioteca para descargar o eliminar.
            </p>
        </div>
    </div>
    ...
    @forelse ($files as $file)
        ...
        <div
            wire:key="helpdesk-video-tutorial-file-{{ $file->getKey() }}"
...
levo                </div>
            ...
            @else
                <div class="flex flex-col items-center justify-center gap-3 rounded-2xl bg-gray-100/90 px-6 py-10 text-center dark:bg-gray-900/60">
                    <span class="rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-800 dark:bg-emerald-500/20 dark:text-emerald-200">
            ...
            <div class="mt-3 flex flex-col gap-3 border-t border-gray-200/80 pt-3 dark:border-white/10 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex flex-wrap justify-end gap-2">
                    <a
                        href="{{ route('helpdesks.video-tutorial-files.download', $file) }}"
                    ...
                    <button
                        type="button"
                        wire:click="mountDeleteHelpdeskVideoTutorialFile({{ $file->getKey() }})"
    ...
    @empty
        <div class="rounded-[1.35rem] border border-dashed border-gray-300/90 bg-white/60 px-4 py-12 text-center text-sm text-gray-500 dark:border-white/15 dark:bg-white/[0.04] dark:text-gray-400">
            <p class="font-medium text-gray-600 dark:text-gray-300">Aún no hay archivos de video tutorial</p>
            <p class="mt-1 text-xs">Carga videos u otros materiales para que estén disponibles en esta biblioteca.</p>
        </div>
    @endforelse
</div>
