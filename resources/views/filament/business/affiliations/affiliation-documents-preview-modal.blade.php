...
@php
    $regenerateAsyncUrl = $affiliation ? route('business.affiliation-documents.regenerate-async', $affiliation) : '';
    $sendEmailUrl = $affiliation ? route('business.affiliation-documents.send-email', $affiliation) : '';
    $sendCarnetEmailsUrl = $affiliation ? route('business.affiliation-documents.send-carnet-emails', $affiliation) : '';
...
@if (! $affiliation)
    <p class="text-sm text-gray-500 dark:text-gray-400">No hay afiliación asociada.</p>
@else
    <div
        wire:ignore
                ...
                <div class="flex flex-col gap-4 bg-gray-50/80 p-4 dark:bg-gray-950/60 sm:flex-row sm:items-end">
                    <div class="min-w-0 flex-1">
                        <label
                            ...
                            for="affiliation-docs-email-{{ $affiliation->getKey() }}"
                        >
                            Correo destino (opcional)
                        </label>
                        <input
                            id="affiliation-docs-email-{{ $affiliation->getKey() }}"
                            type="email"
