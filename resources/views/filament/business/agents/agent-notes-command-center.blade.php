...
<div
    class="fi-agent-notes-command-center space-y-5"
    wire:key="agent-command-center-notes-{{ $record->getKey() }}-{{ $notesWireKey }}"
    x-data="{ draft: '' }"
>
    {{-- Nueva nota (mismo lenguaje visual que notas de tickets) --}}
    <div class="fi-helpdesk-notes-modal space-y-3 px-0.5 py-1">
        <div
            ...
            <form
                class="space-y-3"
                novalidate
                @submit.prevent="$wire.saveAgentCommandCenterNoteFromSlideover('{{ $record->getKey() }}', draft)"
            >
                <textarea
                    x-model="draft"
                    maxlength="255"
                    ...
                    placeholder="Escriba el seguimiento interno para este agente (máx. 255 caracteres)."
                ></textarea>
                <div class="flex flex-wrap items-center justify-between gap-2 text-xs text-gray-500 dark:text-gray-400">
                    ...
                    <button
                        type="submit"
                        wire:target="saveAgentCommandCenterNoteFromSlideover"
                    ...
                    >
                        <span wire:loading.remove wire:target="saveAgentCommandCenterNoteFromSlideover">Guardar nota</span>
                        <span wire:loading wire:target="saveAgentCommandCenterNoteFromSlideover">Guardando…</span>
                    </button>
                </div>
            </form>
        </div>
    ...
    {{-- Bitácora: misma estructura que timeline-modal de helpdesk --}}
    <div class="helpdesk-timeline-modal-root space-y-4 overflow-x-hidden px-1 py-1">
        <section class="rounded-[1.35rem] border border-sky-200/60 bg-sky-50/40 p-4 ring-1 ring-sky-100/80 dark:border-sky-500/25 dark:bg-sky-950/25 dark:ring-sky-500/15">
            <p class="text-sm font-semibold text-sky-950 dark:text-sky-100">Bitácora de notas del agente</p>
        ...
        <section class="rounded-[1.35rem] border border-gray-200/80 bg-white/90 p-4 ring-1 ring-black/[0.04] dark:border-white/10 dark:bg-white/[0.03] dark:ring-white/[0.05]">
            ...
            <p class="mb-4 text-xs text-gray-500 dark:text-gray-400">
                @if ($total === 0)
                    Aún no hay notas registradas para este agente.
                @else
