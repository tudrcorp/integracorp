@php
    /** @var \App\Models\TravelAgency $record */
    use App\Services\TravelAgencyFichaPdfService;

    $codeLabel = TravelAgencyFichaPdfService::codeLabel($record);
    $fichaPreviewUrl = route('business.travel-agencies.ficha-pdf.preview', $record);
    $fichaDownloadUrl = route('business.travel-agencies.ficha-pdf.download', $record);
@endphp
...
<section
    ...
    <div class="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
        <div>
            <p class="text-sm font-semibold text-slate-900 dark:text-white">Ficha de agencia de viajes (PDF)</p>
            <p class="mt-1 max-w-2xl text-xs leading-relaxed text-slate-500 dark:text-slate-400">
                Vista previa con datos de la agencia, agentes y observaciones comerciales. Marca Tu Dr en Casa.
    ...
    <div
        ...
        <iframe
            title="Vista previa ficha de agencia de viajes"
            src="{{ $fichaPreviewUrl }}"
    ...
    <div class="mt-5 grid gap-4 lg:grid-cols-2">
        <div
            ...
            <button
                ...
                @click="$wire.queueTravelAgencyFichaPdfEmail({{ (int) $record->getKey() }}, email)">
                Enviar correo
            </button>
        </div>
        ...
        <div
            ...
            <button
                ...
                wire:target="queueTravelAgencyFichaPdfWhatsApp"
                ...
                @click="$wire.queueTravelAgencyFichaPdfWhatsApp({{ (int) $record->getKey() }}, phone)">
                <span wire:loading.remove wire:target="queueTravelAgencyFichaPdfWhatsApp">Enviar WhatsApp</span>
                <span wire:loading wire:target="queueTravelAgencyFichaPdfWhatsApp">Enviando…</span>
            </button>
        </div>
    </div>
</section>
