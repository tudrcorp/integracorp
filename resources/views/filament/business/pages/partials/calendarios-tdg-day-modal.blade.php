<div
    x-data="{ open: $wire.entangle('isDayModalOpen') }"
    x-show="open"
    x-cloak
        ...
        <header class="shrink-0 border-b border-slate-200/80 px-5 py-4 dark:border-white/10">
            ...
            <div class="mt-4 grid gap-2 sm:grid-cols-3" role="tablist" aria-label="Espacios de la agenda TDG">
                @foreach ([
                    ...
                    <button
                        type="button"
                        wire:click="setModalWorkspace('{{ $workspaceKey }}')"
                        ...
                        role="tab"
                        aria-selected="{{ $modalWorkspace === $workspaceKey ? 'true' : 'false' }}"
                    >
            ...
            @if (in_array($modalWorkspace, ['offices', 'guards'], true))
                <div class="relative mb-4">
                    ...
                    <input
                        ...
                        class="w-full rounded-xl border border-slate-300 bg-white py-2 pl-9 pr-3 text-sm dark:border-white/10 dark:bg-slate-800 dark:text-slate-100"
                    >
                </div>
            @endif
            ...
            @if ($modalWorkspace === 'offices')
                <div class="grid gap-4 lg:grid-cols-3">
                    @foreach ($this->officeOptions as $officeValue => $officeLabel)
                        ...
                        @endphp
                        <div
                            wire:key="tdg-office-card-{{ $officeValue }}"
                            class="rounded-2xl border border-slate-200/80 bg-slate-50/70 p-4 dark:border-white/10 dark:bg-slate-950/40"
                        >
                            ...
                            <div class="mt-4 rounded-xl border border-slate-200/80 bg-white/90 p-3 dark:border-white/10 dark:bg-slate-900/80">
                                <div class="flex items-center justify-between gap-2">
                                    ...
                                    @if ($selectedCount > 0)
                                        <button
                                            type="button"
                                            wire:click="clearOfficeCollaborators('{{ $officeValue }}')"
                                ...
                                @if ($selectedCount > 0)
                                    <div class="mt-2 space-y-2">
                                        @foreach ($selectedCollaborators as $selectedCollaborator)
                                            <div
                                                wire:key="office-selected-{{ $officeValue }}-{{ $selectedCollaborator['id'] }}"
                                                class="flex items-center gap-3 rounded-xl border border-cyan-200/80 bg-cyan-50/50 px-2 py-2 dark:border-cyan-400/30 dark:bg-cyan-500/10"
                                            >
                                                ...
                                                <button
                                                    type="button"
                                                    wire:click="removeOfficeCollaborator('{{ $officeValue }}', {{ $selectedCollaborator['id'] }})"
                                                    class="rounded-lg px-2 py-1 text-[10px] font-semibold text-rose-600 transition hover:bg-rose-50 dark:text-rose-300 dark:hover:bg-rose-500/10"
                                                >
                                                    Quitar
                                                </button>
                            ...
                            <div class="mt-2 max-h-52 space-y-2 overflow-y-auto pr-1">
                                @forelse ($this->filteredCollaboratorOptionsForOffice($officeValue) as $collaborator)
                                    ...
                                    <button
                                        type="button"
                                        wire:key="office-picker-{{ $officeValue }}-{{ $collaborator['id'] }}"
                                        wire:click="assignOfficeCollaborator('{{ $officeValue }}', {{ $collaborator['id'] }})"
            ...
            @if ($modalWorkspace === 'guards')
                <div class="mb-4 rounded-2xl border border-amber-200/80 bg-amber-50/80 px-4 py-3 dark:border-amber-400/30 dark:bg-amber-500/10">
                    <label class="inline-flex items-center gap-2 text-sm font-semibold text-amber-900 dark:text-amber-100">
                        <input
                            type="checkbox"
                            wire:model.live="useSameGuardCollaborator"
                            class="rounded border-amber-400 text-amber-600 focus:ring-amber-400/50"
                        >
                ...
                <div class="grid gap-4 lg:grid-cols-2">
                    @foreach ($this->guardShiftOptions as $shiftValue => $shiftLabel)
                        ...
                        @endphp
                        <div
                            wire:key="tdg-guard-card-{{ $shiftValue }}"
                            class="rounded-2xl border border-slate-200/80 bg-slate-50/70 p-4 dark:border-white/10 dark:bg-slate-950/40 {{ $isSecondaryLocked ? 'opacity-90' : '' }}"
                        >
                                ...
                                @if ($selectedCollaborator)
                                    <div class="mt-2 flex items-center gap-3">
                                        ...
                                        @if (! $isSecondaryLocked)
                                            <button
                                                type="button"
                                                wire:click="clearGuardCollaborator('{{ $shiftValue }}')"
                                                class="rounded-lg px-2 py-1 text-[11px] font-semibold text-rose-600 transition hover:bg-rose-50 dark:text-rose-300 dark:hover:bg-rose-500/10"
                                            >
                                                Quitar
                                            </button>
                                        @endif
                            ...
                            @if (! $isSecondaryLocked)
                                <div class="mt-3 max-h-72 space-y-2 overflow-y-auto pr-1">
                                    @forelse ($this->filteredCollaboratorOptionsForGuardShift($shiftValue) as $collaborator)
                                        ...
                                        <button
                                            type="button"
                                            wire:key="guard-picker-{{ $shiftValue }}-{{ $collaborator['id'] }}"
                                            wire:click="assignGuardCollaborator('{{ $shiftValue }}', {{ $collaborator['id'] }})"
                                            class="flex w-full items-center gap-3 rounded-xl border px-3 py-2 text-left transition
                                            {{ $isSelected
            ...
            @if ($modalWorkspace === 'departments')
                ...
                <div class="flex flex-wrap gap-2">
                    @foreach ($this->departmentOptions as $departmentValue => $departmentLabel)
                        ...
                        <button
                            type="button"
                            wire:click="toggleDepartment('{{ $departmentValue }}')"
                            class="{{ $isSelected ? ($meta['chip_class'] ?? 'tdg-dept-chip tdg-dept-chip--default is-selected') : ($meta['idle_chip_class'] ?? 'tdg-dept-chip tdg-dept-chip--default is-idle') }}"
                        >
        ...
        <footer class="shrink-0 border-t border-slate-200/80 px-5 py-4 dark:border-white/10">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-end">
                <button
                    type="button"
                    wire:click="closeDayModal"
                    class="inline-flex items-center justify-center rounded-xl border border-slate-200/80 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 dark:border-white/10 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800"
                >
                    Cancelar
                </button>
                <button
                    ...
                    wire:target="saveDayAssignments"
                    class="inline-flex items-center justify-center rounded-xl bg-[#0A4174] px-5 py-2 text-sm font-semibold text-[#BDD8E9] transition hover:bg-[#49769F]"
                >
