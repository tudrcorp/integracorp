                ...
                <div class="grid min-h-0 flex-1 grid-cols-1 overflow-hidden lg:grid-cols-12">
                                        ...
                                        <div class="flex flex-wrap items-center justify-between gap-2">
                                            <div class="flex min-w-0 flex-wrap items-center gap-2">
                                                <span class="rounded-full px-2.5 py-1 text-[10px] font-semibold {{ $activityTheme['pill'] }}">
                                                    {{ $activity->department?->label() ?? 'Sin departamento' }}
                                                </span>
                                                <span class="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-semibold text-slate-700 dark:bg-slate-700/80 dark:text-slate-200">
                    ...
                    <main class="h-full overflow-y-auto p-4 pb-8 lg:col-span-8 lg:p-5 lg:pb-10">
                        ...
                        @if ($selectedActivity && $this->canCurrentUserRespondToMeet($selectedActivity) && $currentParticipant)
                            <div class="mb-4 rounded-2xl border border-amber-200/70 bg-amber-50/70 p-4 dark:border-amber-400/30 dark:bg-amber-500/10">
                                <div class="flex flex-wrap items-center justify-between gap-2">
                                    <div class="flex items-center gap-2">
                        ...
                        @if ($isCreatingActivity || $this->canCurrentUserEdit($selectedActivity))
                            <form wire:submit.prevent="saveActivity" class="space-y-4">
                                ...
                                <div class="grid gap-4 sm:grid-cols-2">
                                    ...
                                    <div>
                                        <label class="mb-1 block text-xs font-medium text-slate-600 dark:text-slate-300">Departamento</label>
                                        <select wire:model="activityForm.department" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-xs dark:border-white/10 dark:bg-slate-800 dark:text-slate-100">
                                            <option value="">Seleccione...</option>
                                            @foreach ($this->departmentOptions as $value => $label)
                                                <option value="{{ $value }}">{{ $label }}</option>
                                            @endforeach
                                        </select>
                                        @error('activityForm.department') <p class="mt-1 text-xs text-rose-600">{{ $message }}</p> @enderror
                                    </div>
                            ...
                            <div class="space-y-4">
                                <div class="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-slate-900/80">
                                    ...
                                    <dl class="mt-2 grid gap-2 text-xs text-slate-600 dark:text-slate-300">
                                        <div><span class="font-semibold">Departamento:</span> {{ $selectedActivity->department?->label() ?? 'Sin departamento' }}</div>
                        ...
                        @else
                            <div class="rounded-2xl border border-dashed border-slate-300/80 bg-slate-50 px-4 py-8 text-center dark:border-white/10 dark:bg-slate-900/50">
                                <p class="text-xs text-slate-500 dark:text-slate-400">Selecciona una actividad de la lista o haz clic en «Nueva actividad» para registrar una.</p>
                            </div>
                        @endif
                        @endif
