@php
    ...
    /** @var string|null $generatedAtLabel */
    /** @var string|null $encryptedGeneratedAt */
    /** @var string|null $headerBannerDataUri */
    /** @var string|null $footerBannerDataUri */
    $brandCyan = '#00ADEF';
    $generatedAtLabel = $generatedAtLabel ?? $generatedAt->timezone(config('app.timezone'))->format('d/m/Y H:i');
@endphp
<!DOCTYPE html>
<html lang="es">
<head>
...
<body>
    @if (! empty($headerBannerDataUri))
        <div class="header-banner">
            <img src="{{ $headerBannerDataUri }}" alt="Listado de Proveedores en Venezuela" />
        </div>
    ...
    @if (! empty($footerBannerDataUri))
        <div class="footer-banner">
            <img src="{{ $footerBannerDataUri }}" alt="Tu Doctor en Casa" />
        </div>
    @endif

    <footer class="doc-footer">
        Generado: {{ $generatedAtLabel }} - {{ $encryptedGeneratedAt }} · INTEGRACORP
    </footer>
</body>
</html>
