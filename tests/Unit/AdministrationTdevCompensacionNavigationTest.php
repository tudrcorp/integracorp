...
it('expone página de compensacion de vaucher como vista filament con búsqueda y pestañas', function (): void {
    ...
    expect(file_exists($pagePath))->toBeTrue()
        ->and(file_exists($viewPath))->toBeFalse();
    ...
    expect(file_get_contents($pagePath))
        ...
        ->toContain("->livewireSubmitHandler('searchVouchers')")
        ->toContain("Tab::make('Pago')")
        ->toContain("Tab::make('Estatus')")
        ->toContain("Tab::make('Comisión')")
        ->toContain('EmbeddedTable::make()')
        ->toContain('#[Locked]')
        ...
        ->not->toContain('isResultsModalOpen');
});
