...
it('el menu publico ofrece login de agente y deja afiliaciones pagos y cobros para despues', function (): void {
    $items = StorefrontNav::items(null);
    $keys = array_column($items, 'key');

    expect($keys)->toContain('home', 'quote', 'login', 'business_whatsapp', 'quotes_whatsapp', 'affiliations', 'payments', 'pending')
        ->and($keys)->not->toContain('logout')
        ->and($items[0]['icon'])->toBe('home');

    $soon = array_values(array_filter($items, fn (array $item): bool => $item['soon']));

    expect(array_column($soon, 'key'))->toBe(['affiliations', 'payments', 'pending']);
});
...
it('cada item del menu trae un icono para la hoja inferior', function (): void {
    ...
    expect($icons)->toBe(['home', 'quote', 'login', 'whatsapp', 'whatsapp', 'affiliations', 'payments', 'pending'])
