...
it('la accion generar factura incluye opciones de titular, tomador y personalizada', function (): void {
    ...
    $corporateInvoice = file_get_contents(dirname(__DIR__, 2).'/resources/views/documents/factura-corporativa.blade.php');

    expect($individualInvoice)->toContain("Documento: V-{{ \$data_factura['ci_rif_ti'] }}")
        ->and($corporateInvoice)->toContain("Documento: J-{{ \$data_factura['ci_rif_ti'] }}");
});
...
it('resuelve datos de factura corporativa a nombre del titular y tomador', function (): void {
    ...
    $affiliation = new AffiliationCorporate([
        ...
        'nro_identificacion_contact' => 'V-11223344',
...
it('limpia prefijos V y J del documento antes de enviarlos al pdf', function (): void {
    $sale = new Sale([
        'affiliate_full_name' => 'JUAN TITULAR',
        'affiliate_ci_rif' => 'V-16007868',
        'type' => 'AFILIACION INDIVIDUAL',
    ]);

    $affiliation = new Affiliation([
        'full_name_ti' => 'JUAN TITULAR',
        'nro_identificacion_ti' => 'V16007868',
        'adress_ti' => 'Calle 1',
        'phone_ti' => '04141234567',
        'email_ti' => 'titular@example.com',
        'full_name_payer' => 'PEDRO TOMADOR',
        'nro_identificacion_payer' => 'v-87654321',
        'phone_payer' => '04147654321',
        'email_payer' => 'tomador@example.com',
    ]);

    expect(SalesTable::resolveInvoiceBillingParty('titular', $sale, $affiliation)['ci_rif_ti'])->toBe('16007868')
        ->and(SalesTable::resolveInvoiceBillingParty('tomador', $sale, $affiliation)['ci_rif_ti'])->toBe('87654321');

    $saleWithPlainDocument = new Sale([
        'affiliate_full_name' => 'JUAN TITULAR',
        'affiliate_ci_rif' => '16007868',
        'type' => 'AFILIACION INDIVIDUAL',
    ]);

    expect(SalesTable::resolveInvoiceBillingParty('titular', $saleWithPlainDocument, $affiliation)['ci_rif_ti'])->toBe('16007868');

    $corporateSale = new Sale([
        'type' => 'AFILIACION CORPORATIVA',
    ]);

    $corporateAffiliation = new AffiliationCorporate([
        'name_corporate' => 'CORP SA',
        'rif' => 'J-000111222',
        'address' => 'Zona Industrial',
        'phone' => '02120001111',
        'email' => 'corp@example.com',
        'full_name_contact' => 'ANA CONTACTO',
        'nro_identificacion_contact' => 'J000111222',
        'phone_contact' => '04141112233',
        'email_contact' => 'ana@example.com',
    ]);

    expect(SalesTable::resolveInvoiceBillingParty('titular', $corporateSale, $corporateAffiliation)['ci_rif_ti'])->toBe('000111222')
        ->and(SalesTable::resolveInvoiceBillingParty('tomador', $corporateSale, $corporateAffiliation)['ci_rif_ti'])->toBe('000111222');
});
