...
it('calcula cuatro fechas trimestrales desde la fecha de vigencia', function (): void {
    $effectiveDate = Carbon::create(2026, 2, 16)->startOfDay();

    $dates = AffiliationRenewalCollectionGenerator::upcomingPaymentDates($effectiveDate, 'TRIMESTRAL');

    expect($dates)->toHaveCount(4)
        ->and($dates[0]->format('d/m/Y'))->toBe('16/02/2026')
        ->and($dates[1]->format('d/m/Y'))->toBe('16/05/2026')
        ->and($dates[2]->format('d/m/Y'))->toBe('16/08/2026')
        ->and($dates[3]->format('d/m/Y'))->toBe('16/11/2026');
});
